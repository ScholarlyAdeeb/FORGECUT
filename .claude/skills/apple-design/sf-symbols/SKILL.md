...
(Animation *implementation patterns* live in `apple-design/animation-patterns` → symbol-effects.md;
...
- Full API patterns (SwiftUI `symbolEffect`, UIKit `addSymbolEffect`) in
  `apple-design/animation-patterns/symbol-effects.md`.
...
- Related skills: `apple-design/animation-patterns` (symbol-effects implementation), `apple-design/typography` (SF font pairing), `generators/app-icon-generator` (app icons are NOT symbols)
