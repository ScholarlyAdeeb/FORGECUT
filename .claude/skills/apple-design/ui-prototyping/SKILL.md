...
4. **Tuning panels generalize** beyond animation: swap app states, colors, fonts, visual offsets. Use `apple-design/animation-patterns` for the curve knowledge; the panel is its runnable counterpart.
...
- **Reuses:** `generators/preview-data-generator` (sample content), the `swiftui-builder` agent (edge-case `#Preview` matrix), `apple-design/animation-patterns` (curve reference).
